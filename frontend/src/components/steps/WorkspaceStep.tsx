    ...
    <div className="flex flex-col gap-12 w-full animate-fadeIn">
      ...
      <div className="flex items-center justify-between border-b border-stone-border/40 pb-8">
        ...
        <button
          ...
          className="border border-stone-border hover:bg-stone-muted text-ink-black rounded-full px-5 py-2.5 text-[12px] font-inter font-medium flex items-center gap-1.5 transition-all cursor-pointer active:scale-[0.98] focus:outline-none"
        >
      ...
      <div className="grid grid-cols-12 gap-8 w-full items-start">
        ...
        <div className="col-span-12 lg:col-span-4 flex flex-col gap-6">
          ...
          <div className="bg-pure-white border border-stone-border rounded-xl p-6 shadow-md flex flex-col gap-5">
            <h2 className="text-[12px] font-mono uppercase tracking-wider text-ink-black flex items-center gap-2 border-b border-stone-border pb-3">
            ...
            <div className="flex flex-col gap-1.5 mt-1">
              ...
              <div className="h-3 w-full bg-red-950/20 rounded-full overflow-hidden flex">
                <div
                  className="bg-emerald-500 h-full transition-all duration-300"
                  style={{ width: `${totalLanes > 0 ? (activeLanes / totalLanes) * 100 : 100}%` }}
                />
              </div>
              <div className="flex justify-between text-[10px] font-mono text-warm-gray mt-0.5">
                <span>{totalLanes > 0 ? ((activeLanes / totalLanes) * 100).toFixed(0) : 100}% Active</span>
                <span>{totalLanes > 0 ? ((blockedLanes / totalLanes) * 100).toFixed(0) : 0}% Disabled</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-pure-white border border-stone-border rounded-xl p-6 shadow-md flex flex-col gap-5">
            ...
            <div className="flex flex-col gap-3">
              ...
              <button
                ...
                className={`w-full text-left p-4.5 rounded-lg border transition text-[12px] font-inter flex flex-col gap-1 cursor-pointer active:scale-[0.99] focus:outline-none ${
                  activeTool === 'none'
                    ? 'border-cyan-signal/50 bg-sky-wash/10 text-ink-black font-semibold shadow-inner'
                    : 'border-stone-border bg-pure-white hover:bg-stone-muted/40 text-warm-gray'
                }`}
              >
                ...
                <span className="text-[11px] text-warm-gray font-normal pl-6 leading-relaxed">
              ...
              <div className="flex flex-col gap-3.5 border-t border-stone-border/40 pt-4 mt-2">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {(['circle', 'rect', 'lasso'] as const).map((shape) => (
                    <button
                      ...
                      className={`py-2 rounded-lg border text-[11px] font-inter capitalize transition-all cursor-pointer focus:outline-none ${
                        activeTool === shape
                          ? 'border-cyan-signal/40 bg-sky-wash/15 text-cyan-signal font-semibold'
                          : 'border-stone-border bg-pure-white hover:bg-stone-muted/40 text-warm-gray'
                      }`}
                    >
                      {shape}
                ...
                {activeTool === 'lasso' && isDrawing && (
                  <button
                    ...
                    className="w-full bg-cyan-signal hover:bg-cyan-edge text-stone-canvas hover:text-white py-2 rounded-lg text-[11px] font-inter font-semibold mt-2 flex items-center justify-center gap-1.5 transition active:scale-[0.98]"
                  >
        ...
        <div className="col-span-12 lg:col-span-8 flex flex-col gap-4">
          <div className="bg-pure-white border border-stone-border rounded-2xl p-4 shadow-xl relative min-h-[500px] flex items-center justify-center overflow-hidden">
            {imageUrl ? (
              <div className="relative w-full max-w-[500px] aspect-square rounded-lg border border-stone-border/80 overflow-hidden shadow-lg bg-stone-canvas">
                ...
                {(activeLayer === 'both' || activeLayer === 'satellite') && (
                  <img
                    ...
                    className={`absolute inset-0 w-full h-full object-cover select-none pointer-events-none ${
                      activeLayer === 'both' ? 'brightness-[0.45] contrast-[1.05]' : 'brightness-100'
                    }`}
                  />
                )}
                        ...
                        {activeTool === 'lasso' && (
                          <>
                            {drawPoints.map((p, idx) => (
                              <circle key={`p-${idx}`} cx={p[0]} cy={p[1]} r="4" fill="#06b6d4" />
                            ))}
                            {drawPoints.length > 1 && (
                              <polyline
                    ...
                    {nodes.map((node) => {
                      ...
                      return (
                        <circle
                          ...
                          stroke={isStart || isEnd ? '#ffffff' : '#1e1b4b'}
          ...
          <div className="flex flex-col md:flex-row items-center justify-between bg-pure-white border border-stone-border rounded-xl p-4 gap-4 shadow-md">
            <div className="flex items-center gap-2 text-[12px] font-inter text-warm-gray">
              <Info className="w-4 h-4 text-cyan-signal shrink-0" />
              <span>
                {activeTool === 'none' &&
                  (startNode === null
            ...
            <div className="bg-stone-canvas border border-stone-border rounded-full p-1 flex gap-1 shrink-0">
              {[
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-full text-[11px] font-inter font-medium transition focus:outline-none ${
                    activeLayer === layer.id
                      ? 'bg-pure-white border border-stone-border text-cyan-signal shadow-sm'
