...
export const ExtractionStep: React.FC = () => {
  ...
  useEffect(() => {
    if (graphData && maskB64 && skeletonB64) return // Already loaded, don't re-fetch
    ...
    const triggerExtraction = async () => {
      ...
      try {
        ...
        if (selectedImage) {
          ...
          // If pre-fitted city, fetch the local sample image and convert to File blob
        ...
        // We run in "classical" mode for local mac speed stability, or "ai" if user requests
        const response = await fetch('http://localhost:8000/api/extract-graph?method=classical', {
          method: 'POST',
          body: formData,
        })
  ...
  return (
    <div className="w-full flex flex-col gap-10">
      ...
      <div className="flex items-center justify-between border-b border-stone-border pb-6">
        ...
        <div className="flex items-center gap-4">
          <button
            ...
            className="border border-stone-border hover:bg-stone-canvas text-ink-black rounded-full px-5 py-2.5 text-[12px] font-inter font-medium transition cursor-pointer"
          >
            Cancel Run
          </button>
        </div>
      ...
      <div className="grid grid-cols-4 border border-stone-border rounded-xl divide-x divide-stone-border bg-pure-white overflow-hidden shadow-subtle">
        {[
          ...
          return (
            <div
              ...
              className={`p-4 flex flex-col gap-2 transition-all duration-200 ${
                isCurrent ? 'bg-sky-wash/30' : 'bg-transparent'
              }`}
            >
      ...
      <div className="grid grid-cols-12 gap-8 items-stretch min-h-[500px]">
        {/* Visual Box (Left) */}
        <div className="col-span-12 lg:col-span-8 bg-pure-white border border-stone-border rounded-2xl p-6 shadow-subtle flex items-center justify-center relative overflow-hidden min-h-[400px]">
          {loading && (
            <div className="absolute inset-0 bg-pure-white/80 backdrop-blur-[2px] z-20 flex flex-col items-center justify-center gap-3">
          ...
          {error && (
            <div className="absolute inset-0 bg-pure-white z-20 flex flex-col items-center justify-center gap-4 text-center px-8">
              ...
              <div className="flex flex-col gap-1">
                ...
                <span className="text-[12px] font-inter text-warm-gray max-w-[40ch]">{error}</span>
              </div>
            </div>
          )}

          {imageUrl ? (
            <div className="relative w-full max-w-[500px] aspect-square rounded-lg border border-stone-border overflow-hidden shadow-subtle">
                ...
                <div className="relative w-full h-full">
                  ...
                  <div className="absolute inset-0 grid grid-cols-3 grid-rows-3 gap-0.5 pointer-events-none">
                    {Array.from({ length: 9 }).map((_, i) => (
                      <div
                        key={i}
                        className="border border-cyan-signal/40 bg-cyan-signal/5 animate-pulse flex items-center justify-center text-[10px] font-mono text-cyan-edge font-bold shadow-[inset_0_0_10px_rgba(14,165,233,0.15)]"
              ...
              {activeStep === 2 && maskB64 && (
                <div className="relative w-full h-full bg-soot">
                  ...
                  />
                  <div className="absolute bottom-2 left-2 bg-pure-white/90 backdrop-blur-sm border border-stone-border/40 text-[9px] px-2 py-0.5 rounded font-mono font-medium text-ink-black">
              ...
              {activeStep === 3 && skeletonB64 && (
                <div className="relative w-full h-full">
                  ...
                  {/* High contrast overlay skeleton */}
                  <img
                    src={skeletonB64}
        ...
        <div className="col-span-12 lg:col-span-4 flex flex-col justify-between bg-pure-white border border-stone-border rounded-2xl p-6 shadow-subtle">
          <div className="flex flex-col gap-6">
            <h2 className="text-[13px] font-mono uppercase tracking-wider text-ink-black font-semibold border-b border-stone-border pb-3">
              Extraction Details
            </h2>
              ...
              {graphData && (
                <>
                  ...
                  <div className="flex items-center justify-between border-b border-stone-border/40 pb-2">
                    ...
                    <span className="font-semibold text-rose-500 font-mono">{graphData.edgeCount}</span>
                  </div>
                </>
              )}
            </div>
            ...
            {/* Stepper info cards */}
            <div className="bg-stone-canvas border border-stone-border rounded-xl p-4 flex flex-col gap-2">
          ...
          <div className="flex items-center justify-between border-t border-stone-border pt-6 mt-8">
            <button
              ...
              className="border border-stone-border hover:bg-stone-canvas disabled:opacity-40 disabled:hover:bg-transparent text-ink-black rounded-full px-5 py-2.5 text-[12px] font-inter font-semibold flex items-center gap-1.5 transition cursor-pointer"
            >
            ...
            <button
              ...
              className="bg-cyan-signal hover:bg-cyan-edge disabled:bg-warm-gray text-pure-white rounded-full px-6 py-2.5 text-[12px] font-inter font-semibold flex items-center gap-1.5 transition-all active:scale-[0.98] cursor-pointer"
            >
